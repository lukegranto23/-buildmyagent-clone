...
export default function SignInPage() {
  ...
  useEffect(() => {
    ...
    const messages: Record<string, string> = {
      ...
      EmailSignin: "We could not send the email. Please try again.",
  ...
  const hasGoogleProvider = useMemo(() => Boolean(providers?.google), [providers]);
  ...
  const handleEmailSignIn = async () => {
    ...
    try {
      ...
      if (result?.error) {
        ...
        setStatus("We sent you a sign-in link. Check your inbox and click the button to continue.");
      }
    } catch (err) {
      console.error("Email sign-in failed", err);
      setError("We couldn’t send the magic link. Please try again later.");
    } finally {
        ...
        <div className="max-w-md w-full bg-white rounded-lg shadow-lg p-8">
          ...
          <div className="space-y-4">
            ...
            >
              ...
              {hasGoogleProvider ? "Continue with Google" : "Google sign-in unavailable"}
            </Button>
